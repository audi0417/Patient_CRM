...
router.get('/', async (req, res) => {
  try {
    ...
    const serviceTypes = await queryAll(
      'SELECT * FROM service_types WHERE organizationId = ? ORDER BY displayOrder ASC, createdAt ASC',
      [organizationId]
    );
...
router.get('/active', async (req, res) => {
  try {
    const organizationId = req.user.organizationId;
    const serviceTypes = await queryAll(
      'SELECT * FROM service_types WHERE organizationId = ? AND isActive = 1 ORDER BY displayOrder ASC, createdAt ASC',
      [organizationId]
    );
    res.json(serviceTypes);
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const serviceType = await queryOne(
      'SELECT * FROM service_types WHERE id = ? AND organizationId = ?',
...
router.post('/', async (req, res) => {
  try {
    ...
    const existing = await queryOne(
      'SELECT id FROM service_types WHERE name = ? AND organizationId = ?',
      [name, organizationId]
    );
    if (existing) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // 檢查名稱是否與其他服務類別重複（同組織內）
    const existing = await queryOne(
      'SELECT id FROM service_types WHERE name = ? AND id != ? AND organizationId = ?',
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const usageCount = await queryOne(
      'SELECT COUNT(*) as count FROM appointments WHERE type = (SELECT name FROM service_types WHERE id = ? AND organizationId = ?) AND organizationId = ?',
      [req.params.id, organizationId, organizationId]
    );
    ...
    const result = await execute(
      'DELETE FROM service_types WHERE id = ? AND organizationId = ?',
      [req.params.id, organizationId]
    );
    ...
    await transaction(async (dbAdapter) => {
      for (const item of items) {
        await dbAdapter.execute(
          'UPDATE service_types SET displayOrder = ?, updatedAt = ? WHERE id = ? AND organizationId = ?',
          [item.displayOrder, now, item.id, organizationId]
        );
      }
    });

    const updatedServiceTypes = await queryAll(
      'SELECT * FROM service_types WHERE organizationId = ? ORDER BY displayOrder ASC, createdAt ASC',
      [organizationId]
    );
