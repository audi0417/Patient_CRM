...
const SuperAdminHealthModes = () => {
  const [healthModes, setHealthModes] = useState<HealthMode[]>([]);
  const [analytics, setAnalytics] = useState<Analytics[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [selectedMode, setSelectedMode] = useState<HealthMode | null>(null);
  ...
  const loadHealthModes = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/superadmin/health-modes', {
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error('載入健康模式失敗');
      }
    } catch (error) {
      console.error('Load health modes error:', error);
      toast.error('載入健康模式失敗');
    } finally {
      setIsLoading(false);
    }
  ...
  const loadAnalytics = async () => {
    try {
      const response = await fetch('/api/superadmin/health-modes/usage/analytics', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const loadModeDetail = async (modeId: string) => {
    try {
      const response = await fetch(`/api/superadmin/health-modes/${modeId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const assignModeToOrganization = async () => {
    ...
    try {
      const response = await fetch(`/api/superadmin/organizations/${assignTargetOrg}/health-mode`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">健康模式管理</h1>
          <p className="text-muted-foreground mt-2">
            管理系統預設的健康管理模式，並為組織分配適合的模式
          </p>
        </div>
        ...
        <Dialog open={showAssignDialog} onOpenChange={setShowAssignDialog}>
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>為組織分配健康模式</DialogTitle>
            </DialogHeader>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{healthModes.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="modes" className="space-y-4">
        <TabsList>
          <TabsTrigger value="modes">健康模式</TabsTrigger>
...
export default SuperAdminHealthModes;