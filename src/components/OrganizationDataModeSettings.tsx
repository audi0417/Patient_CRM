...
const OrganizationDataModeSettings = () => {
  const [availableModes, setAvailableModes] = useState<DataMode[]>([]);
  const [currentMode, setCurrentMode] = useState<CurrentMode | null>(null);
  const [selectedMode, setSelectedMode] = useState<string>('');
  const [isLoading, setIsLoading] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [showCustomizeDialog, setShowCustomizeDialog] = useState(false);
  const [customizations, setCustomizations] = useState<CurrentMode['customizations']>({});
  ...
  const loadAvailableModes = async () => {
    try {
      const response = await fetch('/api/data-modes/me/available', {
        headers: {
  ...
  const resetToDefault = async () => {
    setIsSaving(true);
    try {
      const response = await fetch('/api/data-modes/me/reset', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  return (
    <div className="space-y-6">
      {/* 當前模式狀態 */}
      {currentMode && currentMode.modeId ? (
        <Card>
          <CardHeader>
                    ...
                    <div className="space-y-4 py-4">
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          {currentMode.baseConfig?.vitalSignsMapping && 
                           Object.entries(currentMode.baseConfig.vitalSignsMapping).map(([key, field]) => (
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {currentMode.baseConfig?.chartTitles &&
                           Object.entries(currentMode.baseConfig.chartTitles).map(([key, title]) => (
                            <div key={key}>
          ...
          <CardContent>
            <div className="flex items-start gap-4">
              <span className="text-3xl">{currentMode.baseConfig?.icon}</span>
              <div className="flex-1">
                <h3 className="font-semibold text-lg">{currentMode.modeName}</h3>
                <Badge className={getCategoryColor(currentMode.baseConfig?.category || '')}>
                  {currentMode.baseConfig?.category}
                </Badge>
                <p className="text-muted-foreground mt-2">
                  {currentMode.baseConfig?.description}
                </p>
...
export default OrganizationDataModeSettings;